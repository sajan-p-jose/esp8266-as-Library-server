...
labelToStr (char *packet, char *labelPtr, int packetSz, char *res,
	    int resMaxLen)
{
  int i, j, k;
  char *endPtr = NULL;
...
static char ICACHE_FLASH_ATTR *
strToLabel (char *str, char *label, int maxLen)
{
...
#ifndef FREERTOS
static void ICACHE_FLASH_ATTR
captdnsRecv (void *arg, char *pusrdata, unsigned short length)
{
  struct espconn *conn = (struct espconn *) arg;
#else
static void ICACHE_FLASH_ATTR
captdnsRecv (struct sockaddr_in *premote_addr, char *pusrdata,
  ...
  if (espconn_get_connection_info (conn, &remInfo, 0) == ESPCONN_OK)
    {
      ...
      memcpy (conn->proto.udp->remote_ip, remInfo->remote_ip,
	      sizeof (remInfo->remote_ip));
    }
  espconn_sendto (conn, (uint8 *) reply, rend - reply);
#else
  sendto (sockFd, (uint8 *) reply, rend - reply, 0,
  ...
  while (1)
    {
      ...
      ret =
	recvfrom (sockFd, (u8 *) udp_msg, DNS_LEN, 0,
...
{
#ifdef ESP32
printf ("Capt-dns esp32 started");
  xTaskCreate (captdnsTask, (const char *) "captdns_task", 1024, NULL, 3,
	       NULL);
#else

printf ("Capt-dns esp8266 started");

  xTaskCreate (captdnsTask, (const signed char *) "captdns_task", 1024, NULL,
	       3, NULL);
#endif
}
